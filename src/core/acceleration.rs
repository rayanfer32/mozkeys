...
impl AccelParams {
    ...
    ///
    /// `base_speed` and `max_speed` are expressed directly in pixels per tick.
    /// At 240 Hz, 5.0 px/tick = 1200 px/s; 20.0 px/tick = 4800 px/s.
    #[inline]
    pub fn speed(&self, held_us: u64, precision: bool) -> f32 {
        ...
        if precision { v * self.precision_mul } else { v }
    }
}
